...
char* indexes(const char* path)
{
    // convert path from const char* to char*
    char buffer[strlen(path)];
    strcpy(buffer, path);
    // add index to the string
    char* fullNamePHP = strcat(buffer, "index.php");
    char* fullNameHTML = strcat(buffer, "index.html");
    // check if index.php exists in the path
    if ((access(fullNamePHP,F_OK)) == 0 )
    {
        int size = sizeof(char) * strlen(fullNamePHP);
        char* returnPHP = malloc(size);
        strncpy(returnPHP, fullNamePHP, size);
        return returnPHP;
    } 
    // check if index.html exists in the path
    else if ((access(fullNameHTML,F_OK)) == 0 )
    {
        return fullNameHTML;
    }
    // neither exist, return null
    else
    {
        return NULL;
    }
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *content = malloc(capacity);
    ...
    // loop through entire file, reading 512 bytes into a buffer at a time
    // copy buffer into content, reallocing memory for content as necessary
    while ((numberRead = (fread(buffer, 1, 512, file))) > 0)
    {
        if (capacity <= sizeFile + numberRead)
        {
            capacity *= 2;
            *content = realloc (*content, capacity);
        }
        memcpy((*content) + sizeFile, buffer, numberRead);
        sizeFile += numberRead;
        if (ferror(file) != 0)
        {
            *length = 0;
...
const char* lookup(const char* path)
{
    // find the extension, store as string point
    int needle = '.';
    const char* point = strrchr(path, needle);
    
    // compare point to various extensions, return MIME type
    if (strcasecmp(point, ".css") == 0)
    {
        return "text/css";
    }
    if (strcasecmp(point, ".html") == 0)
    {
        return "text/html";
    }
    ...
    if (strcasecmp(point, ".ico") == 0)
    {
        return "image/x-icon";
    }
    if (strcasecmp(point, ".jpg") == 0)
    {
        return "image/jpeg";
    }
    if (strcasecmp(point, ".js") == 0)
    {
        return "text/javascript";
    }
    if (strcasecmp(point, ".php") == 0)
    {
        return "text/x-php";
    }
    if (strcasecmp(point, ".png") == 0)
    {
        return "image/png";
    }
    else
...
{
    // convert fron const char to char
    ...
    // make sure request-target has no "
    else if (strchr(requestTarget, '"') != NULL)
    {
        error(400);
        return false;
    }
    // make sure correct HTTP/1.1 version
    ...
    {
        strncpy(abs_path, requestTarget, strlen(requestTarget) + 1);
        query = "\0";
        return true;
    }
    // if query exists
    ...
    {
        char* abs_pathstore = strtok(requestTarget, "?");
        strncpy(abs_path, abs_pathstore, strlen(abs_pathstore) + 1);
        char* querystore = strtok(NULL,"");
        // if ? is present, but no query
        if (querystore == NULL)
        {
            query = "\0";
        }
        ...
        {
            strncpy(query, querystore, strlen(querystore) + 1);
        }
        return true;
    }
}
